...
def _init_sections_if_needed():
    if (
        "sections" not in st.session_state
        or not isinstance(st.session_state.sections, list)
        or len(st.session_state.sections) == 0
    ):
...
BASE_SAVE_KEYS = {
    ...
    "pas_etrier_r",
    "type_etrier_r",
    "ajouter_second_lit_inf",
    "n_as_inf_2",
    "ø_as_inf_2",
    "jeu_inf_2",
    "ajouter_second_lit_sup",
    "n_as_sup_2",
    "ø_as_sup_2",
    "jeu_sup_2",
    "show_open_uploader",
    ...
    with cev:
        ...
        if v_sup:
            float_input_fr_simple(
                "Effort tranchant réduit V_réduit (kN)", key=K("V_lim", sec_id), default=0.0, min_value=0.0
            )
        else:
...
def render_solicitations_all_sections():
    ...
    def _dimensionnement_compute_states(sec_id: int, beton_data: dict):
        ...
        if mu_key not in beton_data[beton]:
            mu_key = (
                "mu_a500"
                if "mu_a500" in beton_data[beton]
                else [k for k in beton_data[beton].keys() if k.startswith("mu_a")][0]
            )
    ...
    def render_dimensionnement_section(sec_id: int, beton_data: dict):
        ...
        with st.expander(title, expanded=expanded_default):
            ...
            if units_len == "mm":
                tit_h = (
                    f"Vérification de la hauteur : b = {b*10:.0f} mm ; h = {h*10:.0f} mm ; "
                    f"h,min = {hmin_calc*10:.0f} mm"
                )
            else:
                tit_h = f"Vérification de la hauteur : b = {b:.0f} cm ; h = {h:.0f} cm ; h,min = {hmin_calc:.1f} cm"
    
            open_bloc(tit_h, states["etat_h"])
            if units_len == "mm":
                st.markdown(
                    f"**h,min** = {hmin_calc*10:.0f} mm  \n"
            ...
            tit_inf = f"Armatures inférieures : {inf_detail} → As = {As_inf_disp:.2f} {unit_as_txt}"
    
            open_bloc(tit_inf, states["etat_inf"])
            ...
            if has2:
                ...
                with s2c3:
                    float_input_fr_simple(
                        "Jeu entre lits (cm)",
                        key=K("jeu_inf_2", sec_id),
                        default=float(st.session_state.get(K("jeu_inf_2", sec_id), 0.0) or 0.0),
                        min_value=0.0,
                    )
    
            As_inf_total2, inf_detail2 = _as_total_with_optional_second_layer(sec_id, "inf")
            As_inf_disp2 = As_inf_total2 if units_as == "mm²" else As_inf_total2 / 100.0
            st.markdown(
                f"<div style='margin-top:6px;font-weight:600;'>Choix : {inf_detail2} — "
                f"( {As_inf_disp2:.2f} {unit_as_txt} )</div>",
                unsafe_allow_html=True,
            )
            close_bloc()
            ...
            tit_sup = f"Armatures supérieures : {sup_detail} → As = {As_sup_disp:.2f} {unit_as_txt}"
    
            open_bloc(tit_sup, states["etat_sup"])
            ...
            if has2s:
                ...
                with u2c3:
                    float_input_fr_simple(
                        "Jeu entre lits (cm) (sup.)",
                        key=K("jeu_sup_2", sec_id),
                        default=float(st.session_state.get(K("jeu_sup_2", sec_id), 0.0) or 0.0),
                        min_value=0.0,
                    )
    
            As_sup_total2, sup_detail2 = _as_total_with_optional_second_layer(sec_id, "sup")
            As_sup_disp2 = As_sup_total2 if units_as == "mm²" else As_sup_total2 / 100.0
            st.markdown(
                f"<div style='margin-top:6px;font-weight:600;'>Choix : {sup_detail2} — "
                f"( {As_sup_disp2:.2f} {unit_as_txt} )</div>",
                unsafe_allow_html=True,
            )
            close_bloc()
            ...
            if V_val > 0:
                ...
                tit_tau = f"Vérification de l'effort tranchant : τ = {tau:.2f} ≤ {nom_lim} = {tau_lim:.2f}"
                open_bloc(tit_tau, etat_tau)
                ...
                with cE0:
                    st.selectbox(
                        ...
                        index=0
                        if str(st.session_state.get(K("type_etrier", sec_id), "Étriers (2 brins)")) == "Étriers (2 brins)"
                        else (2 if str(st.session_state.get(K("type_etrier", sec_id), "")) == "Étriers (3 brins)" else 1),
                ...
                tit_et = f"Détermination des étriers : pas = {pas:.1f} cm ≤ min({pas_th:.1f}, {s_max:.1f}) = {pas_lim:.1f} cm"
                open_bloc(tit_et, etat_pas)
            ...
            if bool(st.session_state.get(K("ajouter_effort_reduit", sec_id), False)) and V_lim_val > 0:
                ...
                tit_tau_r = f"Vérification effort tranchant réduit : τ = {tau_r:.2f} ≤ {nom_lim_r} = {tau_lim_r:.2f}"
                open_bloc(tit_tau_r, etat_r)
                ...
                with cR0:
                    st.selectbox(
                        ...
                        ["Étriers (2 brins)", "Épingles (1 brin)", "Étriers (3 brins)"],
                        index=0
                        if str(st.session_state.get(K("type_etrier_r", sec_id), "Étriers (2 brins)")) == "Étriers (2 brins)"
                        else (2 if str(st.session_state.get(K("type_etrier_r", sec_id), "")) == "Étriers (3 brins)" else 1),
                ...
                with cR3:
                    float_input_fr_simple(
                        "Pas choisi (cm) (réduit)",
                        key=K("pas_etrier_r", sec_id),
                ...
                tit_et_r = f"Détermination étriers réduits : pas = {pas_r:.1f} cm ≤ min({pas_th_r:.1f}, {s_max_r:.1f}) = {pas_lim_r:.1f} cm"
                open_bloc(tit_et_r, etat_pas_r)
        ...
        with btn4:
            ...
            if st.session_state.get("show_open_uploader", False):
                uploaded = st.file_uploader(
                    "Choisir un fichier JSON", type=["json"], label_visibility="collapsed", key="open_uploader"
                )
        ...
        with input_col_gauche:
            ...
            with csec1:
                st.number_input(
                    ...
                    min_value=5,
                    max_value=1000,
                    value=st.session_state.get("b", 20),
                    step=5,
                    key="b",
                )
            with csec2:
                st.number_input(
                    ...
                    min_value=5,
                    max_value=1000,
                    value=st.session_state.get("h", 40),
                    step=5,
                    key="h",
                )
            with csec3:
                st.number_input(
                    ...
                    min_value=0.0,
                    max_value=100.0,
                    value=st.session_state.get("enrobage", 5.0),
                    step=0.5,
                    key="enrobage",
                )
