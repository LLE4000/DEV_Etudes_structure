...
rougit si on la retire :
  1. POSITIONS PAR DÉFAUT (h = 22, h_pre = 6, Y principale) : la couche 1
     inférieure PRINCIPALE vit dans la peau préfabriquée (règle dalle,
     4,5 cm pour Ø10) ; la couche 1 inférieure SECONDAIRE se pose
     AU-DESSUS de la prédalle (h_pre + demi-Ø = 6,5 cm). Chaque direction
     garde SA hauteur utile : avec My,inf = Mx,inf = 30, la direction
     secondaire demande PLUS d'acier (645 contre 571 mm²).
  2. RENFORT INFÉRIEUR PRINCIPAL (3 Ø12) : posé sur chantier, donc
     AU-DESSUS de la prédalle (7,0 cm) — le CDG pondéré, d et Aₛ,req
     suivent la position réelle.
...
chk("inf. principale (Y, Ø10) DANS la peau préfabriquée : 4,5 cm",
    at.text_input(key="pre1_sec1_dist_axe_inf_y_c1").value == "4,5")
...
chk("My,inf = Mx,inf = 30 : chaque direction garde SON d "
    "(571 en principale, 645 en secondaire)",
    reqs[:4] == ["571", "171", "645", "229"], str(reqs[:4]))
...
chk("CDG pondéré recalculé : (785·4,5 + 339·7,0)/1124 = 5,3 cm",
    at.text_input(key="pre1_sec1_ycdg_inf_y").value == "5,3")
t = md(at)
reqs = re.findall(r"Aₛ,req = (\d+) mm²", t)
chk("Aₛ,req principal suit le d réduit (597 mm²)",
    reqs[0] == "597", str(reqs[:1]))
...
at.run()
chk("X passe dans la peau (4,5), Y au-dessus (6,5)",
    at.text_input(key="pre1_sec1_dist_axe_inf_x_c1").value == "4,5"
...
chk("coupe : trait de clivage annoté (« prédalle préf. » sur le dessin et le pied)",
    t2.count("prédalle préf.") >= 3)
...
chk("chaque direction garde SON d dans la note (16,7 principale / 15,5 secondaire)",
    "16,7" in t2 and "15,5" in t2)
